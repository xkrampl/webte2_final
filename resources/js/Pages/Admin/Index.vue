...
const question = computed(() => props.questions)
...
<template>
    <div class="p-6">
        ...
        <div class="mb-8 p-6 bg-white rounded-lg shadow">
            ...
            <div class="space-y-2">
                <div v-for="user in users" :key="user.id" class="flex items-center justify-between border-b pb-2">
                    <div>
                        <h4 class="font-medium">{{ user.name }}</h4>
                        <p class="text-sm text-gray-600">{{ user.email }}</p>
                    </div>
                    <span :class="{'text-green-500': user.is_admin, 'text-gray-500': !user.is_admin}"
                          class="text-sm font-semibold px-3 py-1 rounded-full bg-gray-100">
